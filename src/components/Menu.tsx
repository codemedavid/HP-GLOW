...
const Menu: React.FC<MenuProps> = ({ menuItems, addToCart, cartItems }) => {
  ...
  const [sortBy, setSortBy] = useState<'name' | 'price' | 'best-sellers'>('name');
  ...
  const sortedProducts = [...filteredProducts].sort((a, b) => {
    // Always prioritize Tirzepatide products
    const aIsTirz = a.name.toLowerCase().includes('tirzepatide');
    const bIsTirz = b.name.toLowerCase().includes('tirzepatide');

    if (aIsTirz && !bIsTirz) return -1;
    if (!aIsTirz && bIsTirz) return 1;
    ...
    switch (sortBy) {
      ...
      case 'best-sellers':
        // Sort by featured (true first), then by name as secondary
        ...
        <div className="container mx-auto px-3 md:px-4 py-4 md:py-6 lg:py-8">
          ...
          <div className="mb-4 md:mb-6 lg:mb-8 flex flex-col sm:flex-row gap-2 md:gap-4">
            ...
            <div className="flex items-center gap-2 md:gap-3 sm:w-auto bg-white rounded-xl md:rounded-2xl px-3 md:px-4 py-1.5 md:py-2 border-2 border-gray-200 shadow-sm">
              ...
              <select
                value={sortBy}
                onChange={(e) => setSortBy(e.target.value as 'name' | 'price' | 'best-sellers')}
